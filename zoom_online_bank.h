...
void phone_format_validation(long long int phone_format);
void phone_validation(long long int phone);
void copy_two_char_array(char receiver[200] ,char transmitter[200] );
void finding_phone_number(long long int tofind);
...
void login(){
    ...
    while (emailExist == -1 || two_charArray==-1){
        ...
        if(emailExist == -1 || two_charArray==-1){
            emailExist=-1;
            two_charArray=-1;
            printf("Email or password was wrong:\n");
        }
    }
    printf("Welcome : %s\n",db[emailExist].name);
...
void userSector(){
    ...
    if(user_option==1){
        ...
        while (amount_to_transfer<db[emailExist].cur_amount){
            ...
            if(db[emailExist].cur_amount-(amount_to_transfer/10+1000)>amount_to_transfer){
                break;
            } else{
...
void transfer_money(int transfer , int receiver , unsigned int amount){
    ...
    db[transfer].cur_amount = db[transfer].cur_amount-(amount + (amount/10));
...
// from-WinHtut-to-lonelone-100-
    unsigned int transfer_fee=amount/10;
    ...
    if(who == 't'){
        ...
        integer_to_char(transfer_fee);
        db[transfer].trc[space_array[transfer]-15].note[index_point]='%';
        index_point++;
        int transfer_fee_counter = charCounting(int_to_char_array_data);
        for(int aaaa=0; aaaa<transfer_fee_counter; aaaa++){
            db[transfer].trc[space_array[transfer]-15].note[index_point]=int_to_char_array_data[aaaa];
            index_point++;
        }
...
void registration(){
    ...
    if( gValid != -1){
        ...
        if( emailExist == -1 ){
            ...
            phone_valid=-1;
            phone_format_check=-1;
            while (phone_valid==-1 || phone_format_check==-1){
                printf("[X]Enter your Phone Number:");
                scanf("%llu",&re_phone);

                phone_validation(re_phone);
                phone_format_validation(re_phone);
...
void nrc_validation(char nrc_toCheck[50]){
    ...
    int nrc_num_check_index=0;

    int num_check_pass=-1;
    int N_check_pass=-1;
    int city_no_check_pass=1;
    ...
    int num_check_counter=0;
    for( int a=nrc_num_check_index; a<nrc_counter ; a++){
        int temp = nrc_toCheck[a];
        if(!(temp >=48 && temp <=57)){
            num_check_pass=-1;
            break;
        }else{
    ...
    if(num_check_counter == nrc_counter - nrc_num_check_index){
        num_check_pass=1;
    }

    nrc_num_check_index--;
    for(int aa=0 ; aa<nrc_counter ;aa++){
        int temp = nrc_toCheck[nrc_num_check_index];
        if(temp >=48 && temp <=57){
            N_check_pass=-1;
            break;
        }else if(nrc_toCheck[nrc_num_check_index]=='/'){
            N_check_pass=1;
            break;
        }else{
        nrc_num_check_index--;
    ...
    for(int aaa=0; aaa<nrc_num_check_index ; aaa++){
        int temp = nrc_toCheck[aaa];
        if(!(temp >=48 && temp <=57)){
            city_no_check_pass=-1;
            break;
        }
    }

    if(num_check_pass==1 && N_check_pass==1 && city_no_check_pass==1){
        nrc_valid=1;
    }
    ...
    for(int i=0; i<SIZE; i++){
        ...
        if(two_charArray == 1){
            printf("Your NRC was already used\n");
            nrc_valid=-1;
            break;
        }
...
void myStrongPassword(char pass[50]){
    ...
    while (strongPass == -1){
        ...
        i++;
        if( special > 0 && numberChar >0 && capChar > 0 && smallChar >0 && count > 7){
            strongPass = 1;
        }
    }
...
void phone_format_validation(long long int phone_format){
    ...
    char phone_format_array[20];
    int index=0;

    FILE *ftpr = fopen("phone_format_temp.txt","w");
    if(ftpr == NULL){
        printf("Error at phone format validation\n");
    }else{
        fprintf(ftpr,"%llu",phone_format);
    }
    fclose(ftpr);

    FILE *ftpr2 = fopen("phone_format_temp.txt","r");
    char c=fgetc(ftpr2);

    while (!feof(ftpr2)){
        phone_format_array[index]=c;
        index++;
        c=fgetc(ftpr2);
    }
    fclose(ftpr2);

    if(index != 10 || phone_format_array[0]!='9' ){
        printf("This is not a phone number!!\n""please input correct ph number\n""Example:09xxxxxxxxx\n");
    }else{
        index=0;
        for(int i=1; i<10 ;i++){
            int to_check =phone_format_array[i];
